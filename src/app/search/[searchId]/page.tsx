...
export async function generateMetadata({ params }: Props) {
  ...
  const db = getDb();
  const [s] = await db.select().from(clipSearches).where(eq(clipSearches.id, searchId)).limit(1);
  return { title: s ? `"${s.query}" — Search` : "Search" };
}
...
export default async function SearchResultPage({ params }: Props) {
  ...
  const [search] = await db.select().from(clipSearches).where(eq(clipSearches.id, searchId)).limit(1);
  if (!search) {
    return <div className="min-h-screen bg-[#09090b] flex items-center justify-center text-[#52525b]">Search not found</div>;
  }

  const results = await db
    .select({ relevanceScore: clipSearchResults.relevanceScore, clip: clipLibrary })
  ...
  const quotes = await db
    .select({ quote: clipSearchQuotes, clip: clipLibrary })
    .from(clipSearchQuotes)
    .innerJoin(clipLibrary, eq(clipLibrary.id, clipSearchQuotes.clipId))
    .where(eq(clipSearchQuotes.searchId, searchId))
    .orderBy(desc(clipSearchQuotes.relevanceScore));
  ...
  const date = search.createdAt.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" });

  return (
    <div className="min-h-screen bg-[#09090b] text-[#d4d4d8]">
      <header className="border-b border-[#18181b] bg-[#0c0c0e]">
        <div className="max-w-5xl mx-auto px-6 py-6">
          <p className="text-[10px] text-[#3f3f46] uppercase tracking-widest mb-1">Topic Search</p>
          <h1 className="text-xl font-semibold text-white">&ldquo;{search.query}&rdquo;</h1>
          <div className="flex gap-4 mt-2 text-xs text-[#52525b]">
            <span><b className="text-[#a1a1aa]">{results.length}</b> clips</span>
            <span><b className="text-[#a1a1aa]">{quotes.length}</b> quotes</span>
            <span>{date}</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-10">
        {/* Quotes — most important, shown first */}
        {quotes.length > 0 && (
          <section>
            ...
            <div className="space-y-3">
              {quotes.map((q) => {
                const secs = Math.floor(q.quote.startMs / 1000);
                const hasTs = q.quote.startMs > 0;
                const ts = hasTs ? `${Math.floor(secs / 60)}:${String(secs % 60).padStart(2, "0")}` : null;
                const link = q.clip.provider === "youtube" && hasTs
                  ? `https://www.youtube.com/watch?v=${q.clip.externalId}&t=${secs}`
                  : q.clip.sourceUrl;

                return (
                  <div key={q.quote.id} className="p-5 rounded-xl bg-[#0f0f12] border-l-3 border-l-amber-500/40 border border-[#18181b]">
                    <p className="text-[16px] text-[#e4e4e7] italic leading-relaxed">
                      &ldquo;{q.quote.quoteText}&rdquo;
                    </p>
                    <div className="flex items-center gap-3 mt-3 text-xs text-[#52525b] flex-wrap">
                      {q.quote.speaker && <span className="text-[#a1a1aa] font-medium">— {q.quote.speaker}</span>}
                      {ts && (
        ...
        {x.length > 0 && (
          <section>
            <h2 className="text-[10px] text-[#3f3f46] uppercase tracking-widest font-semibold mb-4">X / Twitter ({x.length})</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {x.map((r) => (
                <a key={r.clip.id} href={r.clip.sourceUrl} target="_blank" rel="noopener noreferrer" className="p-4 rounded-lg bg-[#0f0f12] border border-[#18181b] hover:border-[#27272a] transition-colors block">
                  <p className="text-xs text-[#d4d4d8] leading-relaxed line-clamp-3">{r.clip.title}</p>
                  <div className="flex items-center gap-3 mt-2 text-[10px] text-[#3f3f46]">
                    <span className="text-sky-400">{r.clip.channelOrContributor}</span>
                    {r.clip.viewCount && <span>{r.clip.viewCount.toLocaleString()} views</span>}
        ...
        {ia.length > 0 && (
          <section>
            <h2 className="text-[10px] text-[#3f3f46] uppercase tracking-widest font-semibold mb-4">Internet Archive ({ia.length})</h2>
            <div className="space-y-2">
              {ia.map((r) => (
                <a key={r.clip.id} href={r.clip.sourceUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 rounded-lg bg-[#0f0f12] border border-[#18181b] hover:border-[#27272a] transition-colors">
                  {r.clip.previewUrl && <img src={r.clip.previewUrl} alt="" className="w-16 h-10 object-cover rounded flex-shrink-0" />}
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-[#d4d4d8] truncate">{decode(r.clip.title)}</p>
                    <span className="text-[10px] text-amber-400">Internet Archive</span>
                  </div>
                  <span className="text-[10px] text-[#27272a]">{r.relevanceScore}/50</span>
                </a>
              ))}
            </div>
          </section>
        ...
        {results.length === 0 && quotes.length === 0 && (
          <p className="text-sm text-[#3f3f46] text-center py-12">No results found</p>
        )}
      </main>
